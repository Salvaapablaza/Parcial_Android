...
class addMasc : Fragment() {
    ...
    private var dbmascotas : DataBase? = null
    ...
    override fun onStart() {
        ...
        dbmascotas = DataBase.getDataBase(addMascview.context)
        mascotaDao = dbmascotas?.MascotaDao()
        ...
        btn_Agregar.setOnClickListener()
        {
            ...
            if (!flag)
            {
                ...
                mascotaDao?.insertMascota(newMascota)

               // listMascotas = addArtArgs.fromBundle(requireArguments()).listArticulos!!.toMutableList()
                listMascotas.add(newMascota)

                send_message(this.addMascview, "Articulo creado satisfactoriamente")
                val directions = addMascDirections.actionAddMascToListFragment()
                addMascview.findNavController().navigate(directions)
            }
        }
